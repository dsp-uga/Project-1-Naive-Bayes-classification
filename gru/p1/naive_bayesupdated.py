...
def run_naive_bayes(data_set):
    ...
    bag_of_words = (train_data).map(calculate_bag_of_words)
    #Calculate Document Prior.
    document_prior = (train_lable).map(calculate_document_prior)
    #Calculate the Document Term Matrix.
    ############################################################################################
    #i have no idea how you want this done. We cannot pass multiple rdd together
    document_prior=sc.broadcast(document_prior)
    bag_of_words=sc.broadcast(bag_of_words)
    document_term_matrix = train_join.map(lambda x:calculate_documente_term_matrix(x,document_prior.value,bag_of_words.value))
    
    #3. Create the Naive-Bayse Model.
    naive_bayes_model = (document_term_matrix).map(naive_bayes)
    ...
    ################################################################################
    #same with this no idea how u want this next thing split
    naive_bayes_model=sc.broadcast()
    predict_naive_bayes = test_data.map(lambda x:predict_naive_bayes(x,document_prior.value,naive_bayes_model.value))
