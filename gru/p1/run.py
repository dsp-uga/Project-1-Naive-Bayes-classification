...
"""
def clean_data(text_data, stop_words):
    ...
    for word in range(0,len(text_data)):
        if(text_data[word] not in stop_words):
...
Here the bag of words  also contains the overall count in the corpus.
...
def calculate_bag_of_words(train_data):
    ...
    bag_of_words = bag_of_words.map(lambda word: (word,1)).reduceByKey(add)
    return bag_of_words
...
def calculate_document_prior(train_lable):
    ...
    document_prior = document_prior.map(lambda lable:(lable[0], lable[1]/total_lables))
...
@param document_prior is the document prior list.
@param bag_of_words is the bag of the words.
@return Document term matrix.
"""
def calculate_documente_term_matrix(train_join,document_prior,bag_of_words):
    #Get the Bag of the words. Words only RDD 
    bag_of_words_wonly = bag_of_words.map(lambda lable: (lable[0]))
    #Get the lables from the Lable priors 
    document_prior_lables = document_prior.map(lambda lable: (lable[0]))
    ...
    #e.g. (('ccat','aaa'),5)
    document_word_count = sc.parallelize(document_word_count.countByKey().items())
...
@param document_term_matrix is the document term matrix or data-dictionary(actually.).
@return naive bayes model which contains P(Wi/Cj), which is probability of each word i in document j.
"""
def naive_bayes(document_term_matrix_rdd):
...
@param document_prior is the document lable prior probabilites.
@param naive_bayes_model gives the word probability given a document. 
...
def predict_naive_bayes(test_data,document_prior,naive_bayes_model):
    ...
    predict_nb_lables = test_data.zipWithIndex().map(lambda text: (text[1],text[0]))
    #Split the text by Space, which would give a pair (index, [list of words])
    predict_nb_lables = predict_nb_lables.map(lambda text:(text[0],text[1].split(" ")))
    #Separate each row in (index,word) format.
    predict_nb_lables = predict_nb_lables.flatMapValues(same_val)
    #Remove the row with the word size less than 2.
    predict_nb_lables = predict_nb_lables.filter(lambda text: len(text[1]) > 1)
    #Pad the pair by 0. Convert into a pair ((index,word),0)
    predict_nb_lables = predict_nb_lables.map(lambda word:(word,0))
    #Sum up the pairs with similar(index,word) key.
    predict_nb_lables = sc.parallelize(predict_nb_lables.countByKey().items())
    ...
    predict_nb_lables = predict_nb_lables.cartesian(document_prior_lables)
    ...
    predict_nb_lables = predict_nb_lables.map(lambda x : ((x[1],x[0][0][1]),(x[0][0][0],x[0][1])))
    #Join the current list with the naive bayes model to convert the data into the desired format.
    predict_nb_lables = predict_nb_lables.join(naive_bayes_model)
    ...
    predict_nb_lables = predict_nb_lables.map(lambda x: ((x[1][0][0],x[0][0]),x[1][0][1] * math.log(x[1][1])))
    #Sum all the log-sum for (index,lable) key. e.g. ((0, 'mcat'), -399.2261060687056)
    predict_nb_lables = predict_nb_lables.reduceByKey(add)
    ...
    predict_nb_lables = predict_nb_lables.map(lambda x : (x[0][1],(x[0][0],x[1])))
    ...
    predict_nb_lables = predict_nb_lables.join(document_prior)
    #Add the doc-prior log probability to the sum. The pair should look like
    #e.g. (0, (-373.3781749241133, 'ccat'))
    predict_nb_lables = predict_nb_lables.map(lambda x : (x[1][0][0],(x[1][0][1] + math.log(x[1][1]),x[0])))
    
    #Now get the maximum log probabilities for the document.
    predict_nb_lables = predict_nb_lables.reduceByKey(max)
    #Keep the (index,lable) onlye. e.g. (0,'ccat') and sort the list.
    predict_nb_lables = sorted(predict_nb_lables.map(lambda x : (x[0], x[1][1])).collect())
    #Convert to RDD.
    predict_nb_lables = sc.parallelize(predict_nb_lables)
    
    return predict_nb_lables
...
def result_analysis(test_lable,predict_naive_bayes):
    ...
    accuracy_list = predict_naive_bayes.join(test_lable_rdd)
...
#     sc = SparkContext.getOrCreate(conf=conf)
    ...
    stop_words = read_stop_words('/home/maulik/stopWords.txt')
    ...
    train_data = train_data.zipWithIndex().map(lambda text:(text[1], clean_data(str(text[0],'utf-8'),stop_words)))
    ...
    test_data = test_data.zipWithIndex().map(lambda text:(text[1], clean_data(str(text[0],'utf-8'),stop_words)))
    ...
    if(data_set != 'large'):
        ...
        #Get the text only.
        test_data = test_data.map(lambda data : data[1]) 

    
    
    #Predict the classes for the test data.
    predict_nb_lables = predict_naive_bayes(test_data,document_prior,naive_bayes_model)
    
#     #5.Calculate the accuracy.
#     if(data_set != 'large'):
#         accuracy = result_analysis(test_lable,predict_nb_lables)
#     else:
#         accuracy = 0 
...
if __name__ == "__main__":
    ...
    with open('output.json', 'w') as fout:
        json.dump(predict_nb_lables.collect(), fout)
